...
def crop_prediction():
    if request.method == "POST":
        try:
            ...
            image_path = f"{prediction[0]}.jpg"
            image_url = url_for('static', filename=f"images/{image_path}", _external=True)

            return jsonify({"prediction": prediction[0], "image_url": image_url})
